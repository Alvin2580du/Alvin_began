...
from models import *
from my_utils import save_image, my_save_image
...
class Trainer(object):
    ...
    def train(self):
        pp.pprint("This is train process, line 138...")
        pp.pprint("self.model_dir: ".format(self.model_dir))
        ...
        for step in trange(self.start_step, self.max_step):
            fetch_dict = {"k_update": self.k_update,
            ...
            if step % (self.log_step * 10) == 0:
                ...
                self.train_autoencode(x_fixed, self.model_dir, idx=step, x_fake=x_fake)
                pp.pprint("go on success ...")
    ...
    def build_model(self):
        ...
        self.z = tf.random_uniform((tf.shape(x)[0], self.z_num), minval=-1.0, maxval=1.0)
        ...
        if self.optimizer == 'adam':
            ...
            raise Exception("[!] Caution! Paper didn't use opimizer other than Adam")
    ...
    def generate(self, inputs, root_path=None, path=None, idx=None, save=True):
        ...
        if path is None and save:
            ...
            save_image(x, path)
            pp.pprint("[*] generate Samples saved: {}".format(path))
        return x

    def test_autoencode(self, inputs, path):
        if inputs.shape[3] in [1, 3]:
            inputs = inputs.transpose([0, 3, 1, 2])
        x = self.sess.run(self.AE_x, {self.x: inputs})
        my_save_image(x, path)

    def train_autoencode(self, inputs, path, idx=None, x_fake=None):
        items = {
            'real': inputs,
            'fake': x_fake,
        }
        for key, img in items.items():
            ...
            save_image(x, x_path)
            pp.pprint("[*] Samples saved: {}".format(x_path))
    ...
    def interpolate_G(self, real_batch, step=0, root_path='.', train_epoch=0):
        ...
        #     my_save_image(img, os.path.join(root_path, 'test{}_interp_G_{}.png'.format(step, idx)), nrow=10)

        all_img_num = np.prod(generated.shape[:2])
        batch_generated = np.reshape(generated, [all_img_num] + list(generated.shape[2:]))
        my_save_image(batch_generated, os.path.join(root_path, 'test{}_interp_G.png'.format(step)), nrow=10)
    ...
    def interpolate_D(self, real1_batch, real2_batch, step=0, root_path="."):
        ...
        #     my_save_image(img, os.path.join(root_path, 'test{}_interp_D_{}.png'.format(step, idx)), nrow=10 + 2)

        all_img_num = np.prod(decodes.shape[:2])
        batch_generated = np.reshape(decodes, [all_img_num] + list(decodes.shape[2:]))
        my_save_image(batch_generated, os.path.join(root_path, 'D_test_interp_{}.png'.format(step)), nrow=10)

    def test(self):
        now_time = datetime.now().strftime('%m.%d_%H%M%S')
        root_path = "./test/{}_{}".format(self.config.model_name, now_time)  # self.model_dir
        if not os.path.exists(root_path):
            ...
            for step in trange(test_epoch):
                ...
                # self.test_autoencode(real1_batch, os.path.join(root_path, 'AE_{}_real1.png'.format(step)))
                # self.test_autoencode(real2_batch, os.path.join(root_path, 'AE_{}_real2.png'.format(step)))

                # z_fixed = np.random.uniform(-1, 1, size=(self.z_num, self.batch_size)).T

                z_fixed1 = np.ones(shape=(self.z_num-127, self.batch_size))
                z_fixed2 = np.zeros(shape=(self.z_num-1, self.batch_size))
                z_fixed = np.concatenate((z_fixed1, z_fixed2), axis=0).T

                # z_fixed = np.concatenate((z_fixed1, z_fixed3, z_fixed2), axis=0).T
                pp.pprint("z_fixed shape: {} ".format(z_fixed.shape))

                G_z = self.generate(z_fixed, path=os.path.join(root_path, "test{}_G_z.png".format(step)))
                my_save_image(G_z, '{}/G_z_{}.png'.format(root_path, step), nrow=int(np.sqrt(self.config.batch_size)))
                ...
                if all_G_z is None:
                    ...
                    # # BEGAN_visualize !
                    # x2_out = self.x2_out.eval(session=self.sess)
                    # print(x2_out.shape)
                    # layer0 = range(len(x2_out))
                    # layer1 = range(len(x2_out[0][0]))
                    # for j in layer0:
                    #     for i in layer1:
                    #         visu_path = "/home/dms/BEGAN_visualize/first_fc_layer/"
                    #         if not os.path.exists(visu_path):
                    #             os.makedirs(visu_path)
                    #         imsave(os.path.join(visu_path, "first_fc_layer_{}_{}.png".format(j, i)), x2_out[j][i])
